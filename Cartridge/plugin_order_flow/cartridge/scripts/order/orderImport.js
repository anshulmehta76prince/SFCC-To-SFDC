...
exports.OrderImport = function(args) {
    ...
    for (var i = 0; i < fileList.length; i++) {
        ...
        var date = new Date(); //construct a date using current timestamp
        var dateTimeStamp = date.getTime();
        var monthFolder = '';
        if (args.ArchiveByMonth) {
            monthFolder = date.getFullYear() + '_' + (date.getMonth() + 1) + '/';
        }
        var folder = new File(File.IMPEX + '/' + args.FilesPath + '/' + args.ArchiveFilesPath + monthFolder).mkdirs(); //create folders (if not exists) (avoid exeption java.nio.file.NoSuchFileException)
        var archivedFile = new File(File.IMPEX + '/' + args.FilesPath + '/' + args.ArchiveFilesPath + monthFolder + file.name.substring(0, file.name.length - 4) + '-' + dateTimeStamp + file.name.substring(file.name.length - 4)); // : File
